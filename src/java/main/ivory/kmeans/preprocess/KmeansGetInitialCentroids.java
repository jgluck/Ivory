...
/**
 * Given a list of document numbers, outputs the documents if it is that document number. Provides initial centroids.
 * 
 * @author Jonathan Gluck
 *
 */
public class KmeansGetInitialCentroids extends PowerTool {
  ...
  /**
   * This mapper takes a file from distributed cache containing valid document numbers
   * and outputs only those documents. It provides the initial centroids for kmeans
   * @author jgluck
   *
   */
  private static class MyMapper extends MapReduceBase implements
    ...
    throws IOException {	
      ...
      if(initialCentroidDocs.contains(mDocno)){
        ...
        output.collect(mDocno, doc); //output the document
      }else{
        ;
      }
...
//    String docnoDir = env.getInitialDocnoDirectory(); //I think I need to change this to the weighted int docvecs
    ...
    //PATH CONTAINING INITIAL CENTROID DOCUMENT NUMBERS
