...
public class KmeansBuildIntPostingsForwardIndex extends PowerTool {
  private static final Logger LOG = Logger.getLogger(KmeansBuildIntPostingsForwardIndex.class);
  ...
  private static class MyReducer extends Reducer<IntWritable, Text, Text, Text> {
    ...
    public void setup(Context context) {
      ...
      positionsFile = env.getPostingsIndexDataEval(conf.getInt("Ivory.currentPack", 0));
  ...
  public KmeansBuildIntPostingsForwardIndex(Configuration conf,int p) {
    super(conf);
    this.pack = p;
  }
  ...
  public int runTool() throws Exception {
    ...
    LOG.info("Tool: " + KmeansBuildIntPostingsForwardIndex.class.getCanonicalName());
    LOG.info(String.format(" - %s: %s", Constants.IndexPath, indexPath));
    LOG.info(String.format(" - %s: %s", Constants.CollectionName, collectionName));

    Job job = Job.getInstance(getConf(),
        KmeansBuildIntPostingsForwardIndex.class.getSimpleName() + ":" + collectionName);
    job.setJarByClass(KmeansBuildIntPostingsForwardIndex.class);

    Path inputPath = new Path(env.getPostingsDirectoryEval(this.pack));
    FileInputFormat.setInputPaths(job, inputPath);

    Path postingsIndexPath = new Path(env.getPostingsIndexDataEval(this.pack));
    ...
    conf.setInt("Ivory.currentPack", this.pack);
